            ...
            <div class="card-header">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    @foreach ($bulan_spp as $data)
                        <li>
                            <a href="{{ route('laporan.presensi.bulanan', $data->id) }}" class="dropdown-item">
                                {{ $data->nama_bulan }}
                            </a>
                        </li>
                    @endforeach
                </ul>
                <button class="btn btn-info dropdown ml-2" type="button" id="dropdownMenu2" data-bs-toggle="dropdown"
                    aria-expanded="false">
                    <i class="metismenu-icon pe-7s-print"></i> DOWNLOAD
                </button>
                <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>
                        <a href="{{ route('export.laporan.bulanan.pdf', $bulan->id) }}" target="_blank"
                            class="dropdown-item">PDF</a>
                    </li>
                    <li>
                        <a href="{{ route('export.laporan.bulanan.excel', $bulan->id) }}" target="_blank"
