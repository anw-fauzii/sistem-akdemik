...
@section('title')
    <title>Edit Target Capaian T2Q</title>
@endsection
    ...
    <div class="app-main__inner">
        <div class="app-page-title mb-4">
            <div class="page-title-wrapper">
                <div class="page-title-heading">
                    ...
                    <div>Edit Target Capaian T2Q
                        <div class="page-title-subheading">
                            Mengubah standar target jilid dan hafalan untuk tingkat kelas tertentu.
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="main-card card border-0 shadow-sm">
            <div class="card-header bg-white pt-4 pb-3 border-bottom-0">
                Form Edit Target Capaian
            </div>
            <div class="card-body">
                <form method="post" action="{{ route('target-capaian-tahsin.update', $targetCapaian->id) }}"
                    id="editForm">
                    @csrf
                    @method('PUT') <div class="form-row">
                        <input type="hidden" name="tahun_ajaran_id" value="{{ $targetCapaian->tahun_ajaran_id }}">
                        ...
                        <div class="col-md-12">
                            <div class="position-relative form-group">
                                ...
                                <input type="text"
                                    value="{{ $targetCapaian->tahunAjaran->nama_tahun_ajaran ?? '' }} - {{ $targetCapaian->tahunAjaran->semester ?? '' }}"
                        ...
                        <div class="col-md-12">
                            <div class="position-relative form-group">
                                ...
                                <input name="tingkat" id="tingkat" placeholder="Misal: 1, 2, 3, PG, A, B" type="text"
                                    ...
                                    value="{{ old('tingkat', $targetCapaian->tingkat) }}" required>
                                @error('tingkat')
                                    <div class="invalid-feedback" style="font-style: italic; font-size: 0.75rem;">
                                        {{ strtolower($message) }}
                                    </div>
                        ...
                        <div class="col-md-12">
                            <div class="position-relative form-group">
                                ...
                                <select name="daftar_jilid_id" id="daftar_jilid_id"
                                    ...
                                    required>
                                    <option value="" disabled>-- Pilih Target Jilid --</option>
                                    @foreach ($daftarJilid as $jilid)
                                        <option value="{{ $jilid->id }}"
                                            {{ old('daftar_jilid_id', $targetCapaian->daftar_jilid_id) == $jilid->id ? 'selected' : '' }}>
                            ...
                            <div class="position-relative form-group">
                                ...
                                <select name="surah_alquran_id" id="surah_alquran_id"
                                    ...
                                    @foreach ($surahAlquran as $jilid)
                                        <option value="{{ $jilid->id }}"
                                            {{ old('surah_alquran_id', $targetCapaian->surah_alquran_id) == $jilid->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group mt-4 mb-0">
                        ...
                        <button type="submit" class="btn btn-primary shadow-sm" id="submitBtn">
                            <i class="pe-7s-diskette mr-1"></i> Update Target
                        </button>
                    </div>
                </form>
            </div>
