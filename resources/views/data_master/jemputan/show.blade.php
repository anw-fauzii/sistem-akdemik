...
<div class="app-main__inner">
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>Anggota Jemputan {{$jemputan->driver}}
    ...
    <div class="main-card card">
        <div class="card-header">
            ...
            @include('data_master.jemputan.modalCreate')
        </div>
        <div class="card-body">
                ...
                <table class="mb-0 table table-hover table-striped" id="myTable2">
                    ...
                    <tbody>
                        @php
                            $no = 1;
                        @endphp
                        @forelse ($anggota_jemputan as $item)
                            <tr>
                                <td>{{$no++}}</td>
                                <td>{{$item->anggotaKelas->siswa->nama_lengkap}}</td>
                                <td>{{$item->anggotaKelas->kelas->nama_kelas}}</td>
                                <td class="d-flex">
                                    <form action="{{ route('anggota-jemputan.destroy', $item->id) }}" method="POST" class="delete-form">
                                        @csrf
                                        @method('DELETE')
