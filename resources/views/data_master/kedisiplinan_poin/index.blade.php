    ...
    <div class="app-main__inner">
        <div class="app-page-title">
            <div class="page-title-wrapper">
                <div class="page-title-heading">
                    ...
                    <div>Aturan Kedisiplinan
                        <div class="page-title-subheading">
                            Merupakan data master untuk aturan kedisiplinan yang akan digunakan pada saat penilaian
                            kedisiplinan siswa.
                        </div>
                    </div>
                </div>
        ...
        <div class="main-card card">
            <div class="card-header">
                <a href="{{ route('kedisiplinan-poin.create') }}" class="btn btn-primary">Tambah Baru</a>
            </div>
            <div class="card-body">
                    ...
                    <table class="mb-0 table table-hover table-striped" id="myTable2">
                        ...
                        <tbody>
                            @php
                                $no = 1;
                            @endphp
                            @forelse ($kedisiplinan_poin as $item)
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $item->kategori }}</td>
                                    ...
                                    <td>{{ $item->poin }}</td>
                                    <td class="d-flex">
                                        <a href="{{ route('kedisiplinan-poin.edit', $item->id) }}"
                                            class="btn btn-sm btn-primary mx-1"><i class="pe-7s-note"
                                                style="font-size: 0.85rem;"></i></a>

                                        <form action="{{ route('kedisiplinan-poin.destroy', $item->id) }}" method="POST"
                                            class="delete-form">
                                            @csrf
                                            @method('DELETE')
