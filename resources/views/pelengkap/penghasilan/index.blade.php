...
<div class="app-main__inner">
            ...
            <div class="page-title-heading">
                ...
                <div>Penghasilan
                    <div class="page-title-subheading">
                        Merupakan kategori untuk jenis Penghasilan
                    </div>
                </div>
            </div>  
        </div> 
    ...
    <div class="main-card card">
        <div class="card-header">
            <a href="{{route('penghasilan.create')}}" class="btn btn-primary">Tambah Baru</a>
        </div>
        <div class="card-body">
                ...
                <table class="mb-0 table table-hover table-striped" id="myTable2">
                    ...
                    <tbody>
                        ...
                        @forelse ($penghasilan as $item)
                            <tr>
                                <td>{{$no++}}</td>
                                <td>{{$item->nama_penghasilan}}</td>
                                <td class="d-flex">
                                    <a href="{{ route('penghasilan.edit', $item->id) }}" class="btn btn-sm btn-primary mx-1"><i class="pe-7s-note" style="font-size: 0.85rem;"></i></a>
                                
                                    <form action="{{ route('penghasilan.destroy', $item->id) }}" method="POST" class="delete-form">
                                        @csrf
                                        @method('DELETE')
