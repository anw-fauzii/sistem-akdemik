    ...
    <div class="app-main__inner">
        <div class="app-page-title">
            <div class="page-title-wrapper">
                <div class="page-title-heading">
                    ...
                    <div>Administrasi Guru
                        <div class="page-title-subheading">
                            Daftar administrasi guru yang telah diupload
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="main-card card">
            <div class="card-header">
                <a href="{{ route('administrasi-guru.create') }}" class="btn btn-primary">Tambah Baru</a>
            </div>
            <div class="card-body">
                    ...
                    <table class="mb-0 table table-hover table-striped" id="myTable2">
                        ...
                        <tbody>
                            ...
                            @forelse ($kategori as $item)
                                @php
                                    $files = $item->administrasiGuru ?? ($item->administrasi_guru ?? collect());
                                    $total = $files->count();
                                    $sudah = $files->where('status', 1)->count();
                                    $belum = $files->where('status', 0)->count();
                                @endphp

                                @include('administrasi.guru.modalCreate')
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <div class="badge badge-sm badge-pill badge-success">{{ $sudah }}
                                                Diperiksa</div>,
                                            <div class="badge badge-sm badge-pill badge-warning">{{ $belum }} Belum
                                            </div>
                                        @endif
                                    </td>
                                    <td class="d-flex">
                                        <button type="button" class="btn btn-sm btn-primary mx-1"
                                            onclick="showCreateModal({{ $item->id }})">
                                            <i class="pe-7s-info" style="font-size: 0.85rem;"></i> Detail
                                        </button>
                                    </td>
                                </tr>
                            @empty
